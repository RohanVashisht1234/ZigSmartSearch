...
pub const SearchLib = struct {
    ...
    pub fn init(allocator: Allocator, db_content: []const u8, config: SearchConfig) !SearchLib {
        // Parse JSON database
    ...
    // Clean up resources
    pub fn deinit(self: *SearchLib) void {
        self.parsed.deinit();
        self.dict.deinit();
        self.lemma.deinit();
    }
    ...
    fn preprocess(self: *SearchLib, input: []const u8) !std.ArrayList([]const u8) {
    ...
    pub fn search(self: *SearchLib, query: []const u8) !std.ArrayList(SearchResult) {
